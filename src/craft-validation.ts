...
export function validateCraftUrl(url: string): void {
  ...
  if (!parsed.pathname.includes('/mcp')) {
    throw new Error(`Invalid Craft MCP URL. Path must include /mcp, got: ${parsed.pathname}`);
  }

  // Must use HTTPS
  if (parsed.protocol !== 'https:') {
    throw new Error(`Invalid Craft MCP URL. Must use HTTPS, got: ${parsed.protocol}`);
  }
}
