---
summary: 'Reference for craft call argument styles, CLI signatures, and structured output modes.'
read_when:
...
`craft call` now understands two complementary styles:

| Style | Example | Notes |
|-------|---------|-------|
| Flag-based (compatible) | `craft call linear.create_comment --issue-id LNR-123 --body "Hi"` | Use `key=value`, `key:value`, or `key: value` pairs—ideal for shell scripts. |
| Function-call (expressive) | `craft call 'linear.create_comment(issueId: "LNR-123", body: "Hi")'` | Mirrors the pseudo-TypeScript signature shown by `craft tools`; unlabeled values map to schema order. |
| Structured output | `craft call 'linear.create_comment(...)' --output json` | Successful calls emit JSON bodies; failures emit `{ server, tool, issue }` envelopes so automation can react to auth/offline/http errors. |
...
`craft tools <connection>` prints each tool as a compact TypeScript declaration:
...
- Run `craft tools <connection> --all-parameters` whenever you want the full signature; the footer repeats `Optional parameters hidden; run with --all-parameters to view all fields.` any time truncation occurs.
...
- **Named arguments preferred**: `issueId: "123"` keeps calls self-documenting. When labels are omitted, craft falls back to positional order defined by the tool schema.
- **Optional positional fallback**: omit labels when calling `craft 'context7.resolve-library-id("react")'`—arguments map to the schema order after any explicitly named parameters.
...
- You can skip `--server` entirely by pasting the MCP endpoint + tool name directly: `craft call 'https://www.shadcn.io/api/mcp.getComponent(component: "vortex")'`.
- Craft strips the `.tool` suffix to derive the base server URL, reuses any configured definition that already points at that endpoint, and only registers a new ad-hoc server when necessary (`--allow-http` still guards plain HTTP URLs).
...
- When in doubt, run `craft tools <connection>` to see the current signature and sample invocation.
